// German translations for dev.geopjr.Hashbrown package.
// Copyright (C) 2021 THE dev.geopjr.Hashbrown'S COPYRIGHT HOLDER
// This file is distributed under the same license as the dev.geopjr.Hashbrown package.
// Philipp Kiemle <philipp.kiemle@gmail.com>, 2021.
// Panagiotis Vasilopoulos <hello@alwayslivid.com>, 2021.

// Last-Translator: Panagiotis Vasilopoulos <hello@alwayslivid.com>
// Language: de

module.exports = {
  "About Hashbrown": {
    comment: "This is a button in the menu dropdown.",
    translation: "Info zu Hashbrown",
  },
  "Generate, compare and verify MD5, SHA-256 & SHA-1 hashes.": {
    comment: "Subtext in welcomer",
    translation:
      "Erzeugen, vergleichen und Verifizieren Sie MD5-, SHA-256- und SHA-1-Hashes.",
  },
  "Select a File": {
    comment: "Tooltip for the main file chooser.",
    translation: "Datei auswählen",
  },
  "Choose a File": {
    comment: "Title for the main file chooser dialog.",
    translation: "Datei auswählen",
  },
  Copy: {
    comment: "A copy text button.",
    translation: "Kopieren",
  },
  Generate: {
    comment: "Header tab about generating hashes.",
    translation: "Erzeugen",
  },
  "Select Another File": {
    comment: "Tooltip for the second file chooser under the compare tab.",
    translation: "Wählen Sie eine weitere Datei",
  },
  "Choose Another File": {
    comment: "Title for the 'compare' file chooser dialog.",
    translation: "Wählen Sie eine weitere Datei aus",
  },
  Compare: {
    comment:
      "Label of a button that compares two files. Also header tab about comparing files (their hashes).",
    translation: "Vergleichen",
  },
  "Insert a MD5/SHA-1/SHA-256 Hash": {
    comment: "Tooltip for the hash text input under the verify tab.",
    translation: "Fügen Sie einen MD5/SHA-1/SHA-256-Hash ein",
  },
  "MD5/SHA-1/SHA-256 Hash": {
    comment:
      "Placeholder text for an input box. Only translate 'hash' if possible.",
    translation: "MD5/SHA-1/SHA-256-Hash",
  },
  Verify: {
    comment:
      "Label of a button that  verifies that a hash belongs to a selected file. Also header tab about verifing that a hash belongs to a selected file.",
    translation: "Verifizieren",
  },
  Menu: {
    comment: "The headbar menu icon-button tooltip.",
    translation: "Menü",
  },
  "They match!": {
    comment: "Message displayed when comparison or verification is successful.",
    translation: "Sie passen zusammen!",
  },
  "They don't match!": {
    comment:
      "Message displayed when comparison or verification is not successful.",
    translation: "Sie passen nicht zusammen!",
  },
};
